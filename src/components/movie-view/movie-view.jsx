...
export function MovieView(props) {
  ...
  const handleAddFavouriteMovie = (e) => {
    ...
    axios
      .post(
        `https://myflix-bna0.onrender.com/users/${UserID}/movies/${movie._id}`,
        {},
        {
  ...
  const handleRemoveFavouriteMovie = (e) => {
    ...
    axios
      ...
      .then((response) => {
        const data = response.data;
        alert(
          `${movie.Title} was successfully removed from your favourites list!`
        );
        location.replace(
          `https://myflix-react.web.app/users/${UserID}/my-list`
        );
      })
      .catch((e) => {
  ...
  return (
    <>
      <Card className="p-3 movie-view-card">
        ...
        />
        <Card.Body>
          <Card.Title className="movie-view-card-title">
            {movie.Title}
          </Card.Title>
          <Card.Text className="movie-view-card-text">
            {movie.Description}
          </Card.Text>
          <Card.Text className="movie-view-card-text">
        ...
        <Row className="movie-view-card-favourite-btns">
          <Link to={`/movies/${movie.Title}`}>
            <Button
              ...
              type="submit"
              onClick={handleAddFavouriteMovie}
            >
              Add to My List
            </Button>
          </Link>
          <Link to={`/movies/${movie.Title}`}>
            <Button
              ...
              type="submit"
              onClick={handleRemoveFavouriteMovie}
            >
              Remove from My List
            </Button>
          </Link>
